...
# ===== ベースディレクトリ =====
BASE_ROOT = r"C:\Users\ishiz\Documents\Akamine_workspace\master_thesis_2025\SMR_control\IK_only_result"
...
def open_camera(index=0, width=1920, height=1080):
    """
    カメラを1回だけオープンして返す。
    その後は capture_frame(cap) でフレームだけ取る。
    """
...
def create_mse_plot_and_gif(
    ...
    - overlay_paths: compute_mse_and_overlay で保存したオーバーレイ画像のパス
      を使って、
        - mse_time_series.png        （ステップ軸）
        - mse_time_series_time.png   （時間[s]軸・step版と同じshift）
    ...
    plt.xlim(0, NUM_LOOP)  # ★ 0〜NUM_LOOP → x=-1 の点は見えない
    plt.tight_layout()
    ...
    # ---- MSEプロット (時間[s]軸：step版と同じshift) ----
    dt = WAIT_TIME + LOOP_WAIT   # 1step あたりの実時間 [sec]
    time_axis = [s * dt for s in steps_shifted]   # -dt, 0, dt, 2dt, ...
    ...
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.xlim(0, NUM_LOOP * dt)   # ★ ここでも -dt の点は見えない
    plt.tight_layout()
...
def plot_voltage_bars_for_step(target_volt, pred_volt, out_path, step_idx=None):
    """
    target_volt: shape (M,2)
    pred_volt: shape (M,2)
    step_idx: ステップ番号 (1-based)
    """
...
def create_voltage_bar_gif(bar_image_paths, save_dir,
    ...
    """
    duration:
        - float: すべてのフレームの表示時間（秒）
        - list[float]: 各フレームごとの表示時間（秒）
    """
    frames = []
    ...
    frames[0].save(
        ...
        save_all=True,
        append_images=frames[1:],
        duration=durations_ms,  # ← リストまたは単一値どちらもOK
        loop=0
    )
...
def run_feedback_loop(
    ...
    initial_voltages,        # フィードバック開始時点の電圧 (N,2), すでに [0,5] に clip 済みを想定
    first_cam_frame,         # フィードバック開始時点のフルフレーム（BGR）
    ...
    base_save_dir,           # save_dir（selected_xxx）
    num_loops=30,
    wait_sec=5.0,
    start_step_idx=2,        # MSE / overlay のタイムステップ開始インデックス
    ...
    cap=None,                # ★ open_camera で開いた VideoCapture
    target_voltages=None,    # ★ 追加: (M,2)[Left,Right] 目標電圧（存在モジュール分）
):
    """
    0V など任意の初期電圧状態から、IKBeamNet を用いて指定回数フィードバックを回す。
    ...
    ・各ループ：
        ...
        - current_voltages（[0,5]V）を q_map として結合 → (6ch,H,W) を IKBeamNet に入力
        ...
        - ログには「V_i_clip」「V_next_clip」「V_ik_raw」を出す
        - wait_sec 秒待ってカメラ撮影 → 次ループへ
        - 各ループごとに fb_step_xx/ フォルダを作ってログを保存
        - ★ target_voltages があれば、各ステップで pred vs target の棒グラフ画像も保存
    """
    ...
    # デバッグ用：ターゲットの center_crop と beammask を保存
    ...
    mse_list_fb = []
    overlay_paths_fb = []
    bar_paths_fb = []   # ★ 追加: 各ステップの棒グラフ画像パスを溜める
    ...
    for loop in range(num_loops):
        ...
        for m_idx in range(effective_modules):
            ...
            # ログ出力（clip済みの V_i と V_next、IK の生値）
            print(
        ...
        # ★ 各ステップの pred vs target 棒グラフを保存（target_voltages がある場合）
        if target_voltages is not None:
            ...
            # signals.csv 仕様により、存在しないモジュールには 0 が入っているので除外
        ...
        # 次のカメラフレーム取得（既に open 済みの cap から）
    ...
    return mse_list_fb, overlay_paths_fb, bar_paths_fb  # ★ 3つ返す
...
def main():
    ...
    # ===== signals.csv から目標電圧 (target_voltages) を取得 =====
    target_voltages = None
    try:
        img_p = Path(img_path)
        module_dir = img_p.parent.parent  # .../1module_dataset_max_DAC/roi/NN.png → その一つ上が dataset dir
        ...
        if signals_path.exists():
            ...
            # img_name は "1", "2", ... の想定 → CSV は 1行目が 1.png に対応
            try:
                row_idx = int(img_name) - 1  # 0-based index
            except ValueError:
            ...
            if row_idx is not None and 0 <= row_idx < len(signals_df):
                ...
                if row.shape[0] >= 12:
                    ...
                    left = row[0:6]   # (6,)
                    right = row[6:12] # (6,)
                    lr = np.stack([left, right], axis=1)  # (6,2)

                    num_modules_detected = boxes.shape[0]
                    lr = lr[:num_modules_detected, :]  # devnetで検出された数に合わせる

                    # この時点では 0V の行（存在しないモジュール）は残しておく
                    # run_feedback_loop 内で 0 行は除外して棒グラフを描く
                    target_voltages = lr
    ...
    try:
        ...
        if cam_frame_init is None:
            ...
            mse_fb, overlays_fb, bar_paths_fb = run_feedback_loop(
                ...
                cap=cap,            # ★ カメラハンドルを渡す
                target_voltages=target_voltages,  # ★ 追加
            )
