# -*- coding: utf-8 -*-
"""
eval_module_controller_vae.py

- train_module_controller_vae.py で学習した VAEControllerNet を読み込んで評価する
- MSE / MAE (全体 + 左右別) を計算
- 予測 vs 正解 の散布図を PNG 出力
- 誤差ヒストグラムも出力
- 元画像 ＋ 再構成画像のグリッドを PNG 保存
- 潜在空間 (mu) の分布を可視化：
    - 各次元ごとのヒストグラム
    - mu[0] vs mu[1] の散布図
    - PCA による 2 次元射影の散布図
- 実際にサンプリングされた潜在表現 z の PCA 2次元射影も可視化：
    - PCA による 2 次元射影の散布図 (latent_z_pca2d.png)
"""
...
# 評価結果の保存先
EVAL_DIR = Path(os.path.dirname(MODEL_SAVE_PATH)) / "eval_vae"
